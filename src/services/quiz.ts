...
export const fetchCategories = async () => {
  const res = await openTdbAPI.get<ICategoryAPIResponse>(
    apiRoutes.quiz.fetchCategories
  );
...
export const fetchQuestions = async (
  ...
  const res = await openTdbAPI.get<IQuestionsAPIResponse>(
    apiRoutes.quiz.fetchQuestions,
    {
      params: {
  ...
  return res.data.results.map(
    ({ question, correct_answer, incorrect_answers }) => ({
      ...
      options: shuffleArray([...incorrect_answers, correct_answer]).map(
        decodeHtmlCharCodes
      ),
    })
  );
